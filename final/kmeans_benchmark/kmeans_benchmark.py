...
class LMI:
    ...
    def _visit_buckets(
        ...
        for nth_bucket in range(nprobe):
            D, I = self._visit_bucket(
                int(predicted_buckets[nth_bucket].item()), query, k
            )
        ...
        with ThreadPoolExecutor(max_workers=9) as executor:
            results = executor.map(
                lambda i: self._visit_buckets(
                    k, predicted_bucket_ids[i], queries[i : i + 1], i, nprobe
                ),
                range(n_queries),
            )
    ...
    def _bucket_init(self, classes: Tensor, bucket: int) -> None:
        ...
        self.bucket_data[bucket] = torch.empty(
            (len(indices), self.dimensionality), dtype=torch.float16
        )
    ...
    @staticmethod
    def _run_kmeans(
        n_buckets: int, data_dim: int, X_train: Tensor, method
    ) -> Tensor:
        if method == "faiss" or method == "faiss_non_spherical":
            kmeans = faiss.Kmeans(
                d=data_dim,
                k=n_buckets,
                verbose=False,
                seed=SEED,
                spherical=method != "faiss",
            )
            kmeans.train(X_train)
            return torch.from_numpy(kmeans.index.search(X_train, 1)[1].T[0])  # type: ignore
        elif method == "lmi":
            return LMI_RS._run_kmeans(n_buckets, data_dim, X_train)
    ...
    def create(
        ...
        y = LMI._run_kmeans(n_buckets, data_dim, X_train, kmeans_method)
...
def task1(
    ...
    start = time.time()
    lmi = LMI.create(dataset, epochs, lr, sample_size, n_buckets, chunk_size, kmeans_method)
    ...
    for nprobe in nprobes:
        ...
        identifier = f"t1-{dataset_size}-epochs={epochs}-lr={lr}-sample={sample_size}-alpha={alpha}-chunk_size={chunk_size}-kmeans_method={kmeans_method}-nprobe={nprobe}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kmeans-method", type=str, default="faiss")
