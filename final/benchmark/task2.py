# Adapted from https://github.com/Coda-Research-Group/LearnedMetricIndex/blob/paper-sisap24-indexing-challenge/task2.py
...
thread_lock_n_candidates = threading.Lock()
n_candidates_per_query = torch.full((10_000,), -1)
...
class LMI:
    ...
    def _visit_bucket(
        self, bucket: int, query: Tensor, k: int
    ) -> tuple[Tensor, Tensor]:
        if len(self.bucket_data[bucket]) == 0:
            return torch.full((k,), float("-inf"), dtype=torch.float32), torch.full(
                (k,), -1
            )
    ...
    def _visit_buckets(
        ...
        for nth_bucket in range(nprobe):
            with thread_lock_n_candidates:
                bucket_id = int(predicted_buckets[nth_bucket].item())
                n_candidates_per_query[query_idx] += len(self.bucket_data[bucket_id])
            D, I = self._visit_bucket(
                int(predicted_buckets[nth_bucket].item()),
                decomposed_query,
                n_candidates,
            )
        ...
        if len(Ds[Ds > 0]) == 0:
            return (
                torch.full((k,), float("-inf"), dtype=torch.float32),
                torch.full((k,), -1),
                query_idx,
            )
    ...
    def _bucket_init(self, classes: Tensor, bucket: int) -> None:
        ...
        self.bucket_data[bucket] = torch.full(
            (len(indices), self.dimensionality), -1, dtype=torch.float16
        )

    def _sort_data(
        self, data: Tensor, classes: Tensor, bucket: int, start: int, stop: int
    ) -> None:
        self.bucket_data[bucket][start:stop] = data[classes == bucket]

    @utils.measure_runtime
    def _label_data(
        self, dataset: Path, chunk_i: int, chunk_size: int
    ...
    def _chunk_sort(
        ...
        start = time.time()
        decomposed_chunk = torch.from_numpy(self.tsvd.transform(chunk)).to(
            torch.float16
        )
    ...
    def _create_offsets(
        self, classes: Tensor, n_chunks: int, chunk_size: int
    ) -> Offsets:
    ...
    def _create_buckets(self, dataset: Path, n_data: int, chunk_size: int) -> float:
        ...
        encdatabasetime = 0.0
        for i in range(n_chunks):
            encdatabasetime += self._chunk_sort(
                dataset, classes, i, offsets, chunk_size
            )
        gc.collect()
    ...
    def create(
        ...
        """dump_path = (
            '/storage/brno12-cerit/home/prochazka/projects/learnedmetricindex-bp/'
            'epochs=15-lr=0.00098-sample=500000-buckets=10000-chunk_size=100000-reduced_dim=110'
        )

        with Path.open(dump_path, "wb") as file:
           lmi: LMI = pickle.load(file)"""
...
def task2(
    ...
    ncandidates: int,
    reduced_dim: int,
    dataset_path: str,  # TODO: remove
) -> None:
    global n_candidates_per_query

    # dataset = Path(f'/storage/brno12-cerit/home/prochazka/datasets/sisap24/laion2B-en-clip768v2-n={dataset_size}.h5')
    ...
    torch.save(
        lmi.bucket_data_ids,
        f"/storage/brno12-cerit/home/lickomar/sisap24-python/bucket_data_ids-t2-{dataset_size}.pt",
    )
    ...
    bucket_sizes = []
    for i in range(len(lmi.bucket_data)):
        bucket_sizes.append(len(lmi.bucket_data[i]))

    bucket_sizes = np.array(bucket_sizes).astype(np.int32)
    ...
    for nprobe in [
        ...
        predicted_buckets = lmi._predict(queries, nprobe).numpy().astype(np.int32)
        ...
        with thread_lock_n_candidates:
            logger.debug(nprobe)
            logger.debug(n_candidates_per_query)

            torch.save(
                n_candidates_per_query,
                f"/storage/brno12-cerit/home/lickomar/sisap24-python/n_candidates_per_query-t2-{dataset_size}-{nprobe}.pt",
            )

        utils.store_results(
            Path(
                "/storage/brno12-cerit/home/lickomar/sisap24-python/result-n_candidates/"
            )
            / "task2"
            / dataset_size
            / f"{identifier}.h5",
