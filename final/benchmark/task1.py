# Adapted from https://github.com/Coda-Research-Group/LearnedMetricIndex/blob/paper-sisap24-indexing-challenge/task1.py
...
class LMI:
    ...
    def create(
        ...
        return lmi, kmeanstime, trainmodeltime
...
def task1(
    ...
    # f'/storage/brno12-cerit/home/prochazka/fi-lmi-data/david/datasets/sisap24/laion2B-en-clip768v2-n={dataset_size}.h5'
    dataset = Path(
        f'/storage/brno12-cerit/home/prochazka/datasets/sisap24/laion2B-en-clip768v2-n={dataset_size}.h5'
    )
    ...
    start = time.time()
    lmi, kmeanstime, trainmodeltime = LMI.create(dataset, epochs, lr, sample_size, n_buckets, chunk_size)
    buildtime = time.time() - start

    torch.save(
        lmi.bucket_data_ids,
        f'/storage/brno12-cerit/home/lickomar/sisap24-python/bucket_data_ids-t1-{dataset_size}.pt',
    )
    ...
    np.save(f'/storage/brno12-cerit/home/lickomar/sisap24-python/bucket_sizes-t1-{dataset_size}.npy', bucket_sizes)

    for nprobe in range(1, 30 + 1):
    # for nprobe in [4]:
        with thread_lock_n_candidates:
            n_candidates_per_query[:] = 0
        
        predicted_buckets = (
            lmi._predict(queries, nprobe).numpy().astype(np.int32)
        )
        ...
        with thread_lock_n_candidates:
            ...
            torch.save(
                ...
                f'/storage/brno12-cerit/home/lickomar/sisap24-python/n_candidates_per_query-t1-{dataset_size}-{nprobe}.pt',
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output-dir', type=str, default='/storage/brno12-cerit/home/lickomar/sisap24-python/result-n_candidates/')
