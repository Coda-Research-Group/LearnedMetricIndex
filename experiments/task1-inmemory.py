...
class LMI:
    ...
    def _predict(self, X: Tensor, top_k: int) -> Tensor:
        ...
        with torch.no_grad():
            predict = self.model(X).topk(top_k)[1]
    ...
    def _sort_data(self, data: Tensor, classes: Tensor, bucket: int) -> None:
        indices = torch.where(classes == bucket)[0]
        self.bucket_data_ids[bucket] = indices
        self.bucket_data[bucket] = data[indices]
    ...
    @utils.measure_runtime
    def _create_buckets(self, X: Tensor, n_data: int, chunk_size: int) -> None:
        ...
        for chunk_i in range(n_chunks):
            start, stop = chunk_i * chunk_size, (chunk_i + 1) * chunk_size
            classes[start:stop] = self._predict(X[start:stop].to(torch.float32), 1).reshape(-1)
        gc.collect()
        logger.debug('Predicted')
        # offsets = self._create_offsets(classes, n_chunks, chunk_size)
        logger.debug("label_t,{}", time.time() - start_t)
        """
        start = time.time()
        with ThreadPoolExecutor() as executor:
        ...
        """
        with ThreadPoolExecutor() as executor:
            executor.map(lambda x: self._sort_data(X, classes, x), range(self.n_buckets))
        gc.collect()
    ...
    def create(
        ...
        X = torch.from_numpy(h5py.File(dataset, 'r')['emb'][:])
        sample_indices = torch.randint(0, n_data, (sample_size,))
        X_train = torch.empty((sample_size, data_dim), dtype=torch.float16)
        ...
        lmi._create_buckets(X, n_data, chunk_size)
...
def task1(
    ...
    for nprobe in nprobes:
        ...
        identifier = f't1_inmemory-{dataset_size}-epochs={epochs}-lr={lr}-sample={sample_size}-alpha={alpha}-chunk_size={chunk_size}-nprobe={nprobe}'
    ...
    logger.debug(f"memory peak: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
