...
class LMI:
    ...
    def _bucket_init(self, classes: Tensor, bucket: int) -> None:
        ...
        self.bucket_data[bucket] = torch.empty((len(indices), self.dimensionality))
    ...
    def _label_data(self, dataset: Path, chunk_i: int, chunk_size: int) -> tuple[Tensor, int, int]:
        ...
        chunk = utils.read_fbin(dataset, start, chunk_size)
    ...
    def _create_buckets(self, dataset: Path, n_data: int, chunk_size: int) -> None:
        ...
        logger.debug(f"memory peak classes: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
        ...
        logger.debug(f"memory peak label: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
        ...
        logger.debug(f"memory peak init: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
        logger.debug('First part done')

        for i in range(n_chunks):
            self._chunk_sort(dataset, classes, i, offsets, chunk_size)
        gc.collect()
        logger.debug(f"memory peak sort: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
    ...
    def create(
        ...
        n_data, data_dim = utils.get_dataset_shape_fbin(dataset)
        X_train = utils.sample_train_subset_1B(dataset, n_data, data_dim, sample_size, chunk_size)
        logger.debug(f"memory peak sample: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")

        logger.debug(f'Training on {X_train.shape[0]} subset from {n_data} dataset')

        y = LMI._run_kmeans(n_buckets, data_dim, X_train)
        logger.debug(f"memory peak kmeans: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
        ...
        logger.debug(f"memory peak train: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
...
def task1(
    ...
    dataset = Path('./base.1B.fbin')
    # dataset = Path(f'/storage/brno12-cerit/home/cernansky-jozef/datasets/base.1B.fbin')
    ...
    for nprobe in nprobes:
        ...
        utils.store_results(
            Path('/storage/brno12-cerit/home/cernansky-jozef/result/') / 'task1' / '1B' / f'{identifier}.h5',
            'lmi',
            D,
            I + 1,
            modelingtime,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset-size', type=str, default='1B')
