...
class LMI:
    ...
    def _train_model(
        ...
        for epoch in range(epochs):
            for X_batch, y_batch in train_loader:
                ...
                logger.debug(f"Chunk debug {time.time() - s}")
    ...
    def create(
        ...
        logger.debug("Normal train start")
        LMI._train_model(nn, X_train, y, epochs, lr)
        logger.debug("Compiled train start")
        LMI._train_model(nn_opt, X_train, y, epochs, lr)
        ...
        y_base = torch.empty((n_data,), dtype=torch.int32)
        y_comp = torch.empty((n_data,), dtype=torch.int32)
        ...
        for chunk_i in range(n_chunks):
            ...
            with torch.no_grad():
                s = time.time()
                y_base[start:stop] = nn(chunk.to(torch.float32)).topk(1)[1].reshape(-1)
                t = time.time() - s
                logger.debug(f"base_t chunk: {t}")
                base_t += t
                s = time.time()
                y_comp[start:stop] = nn_opt(chunk.to(torch.float32)).topk(1)[1].reshape(-1)
                t = time.time() - s
                logger.debug(f"comp_t chunk: {t}")
                comp_t += t

            del chunk

        logger.debug(f'base_t: {base_t}, comp_t: {comp_t}')
        exit()
