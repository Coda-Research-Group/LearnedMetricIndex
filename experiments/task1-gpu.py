...
class LMI:
    ...
    def _train_model(
        ...
        for epoch in range(epochs):
            for X_batch, y_batch in train_loader:
                loss = loss_fn(model(X_batch.to(torch.float32).cuda()), y_batch.cuda())
    ...
    def _visit_bucket(self, bucket: int, query: Tensor, k: int) -> tuple[Tensor, Tensor]:
        ...
        bucket_data = self.bucket_data[bucket].to(torch.float32).numpy()
        res = faiss.StandardGpuResources()
        D, I = faiss.knn_gpu(res, query.numpy(), bucket_data, k, metric=faiss.METRIC_INNER_PRODUCT)
        del bucket_data
    ...
    def search(self, queries: Tensor, k: int, nprobe: int = 100) -> tuple[np.ndarray, np.ndarray]:
        start = time.time()
        self.model.cuda()
        predicted_bucket_ids = self._predict(queries.cuda(), nprobe)
    ...
    def _create_buckets(self, dataset: Path, n_data: int, chunk_size: int) -> None:
        ...
        start = time.time()
        n_gpu_chunks = ceil(n_data / gpu_chunk_size)

        classes = torch.empty((n_data,), dtype=torch.int32)

        for i in range(n_gpu_chunks):
            result = self._label_data(dataset, i, gpu_chunk_size)
    ...
    def create(
        ...
        X_train: Tensor = utils.sample_train_subset(dataset, n_data, data_dim, sample_size, chunk_size)
...
def task1(
    ...
    for nprobe in nprobes:
        ...
        identifier = f't1-gpu-{dataset_size}-epochs={epochs}-lr={lr}-sample={sample_size}-alpha={alpha}-chunk_size={chunk_size}-nprobe={nprobe}'
